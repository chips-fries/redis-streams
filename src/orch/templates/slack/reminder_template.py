...
class SlackReminderTemplate:
    ...
    def __init__(
        ...
        main_text: str,  # Main text (already processed for time/mentions)
        ...
        **kwargs,  # Catch any extra data
        ...
        # Basic validation
        if not all([self.reminder_id, self.recipient, self.main_text]):
            logger.error(
    ...
    def _build_base_blocks(self) -> List[Dict[str, Any]]:
        ...
        blocks = [
            {
                "type": "section",
                "text": {"type": "mrkdwn", "text": self.main_text or "(Reminder)"},
            }
        ]
        context_elements = []
        if self.sub_text:
    ...
    def build_initial_payload(self) -> Dict[str, Any]:
        ...
        fallback_text = (
            f"Reminder [{self.interaction_mode}]: {self.main_text or '(See message)'}"
        )
        if self.sub_text:
    ...
    #     # Build simple text payload for thread follow-up using self.main_text etc.
