...
try:
    ...
    try:
        from _orch.templates.slack.reminder_template import SlackReminderTemplate
    except ImportError:
        SlackReminderTemplate = None
        logging.warning("SlackReminderTemplate not found.")
    try:
        from _orch.templates.line.reminder_template import LineReminderTemplate
    except ImportError:
...
async def build_and_publish_reminder_task(
    ...
    try:
        ...
        replacements = {
            context_data.get("timestamp_placeholder", "{{TIME}}"): now_str,
        ...
        fields_to_process = ["reminder_text", "sub_text"]
        ...
        if mention_ids and isinstance(mention_ids, list) and channel == "slack":
            ...
            if formatted_mention_string:
                ...
                run_logger.debug(
                    f"Prepended Slack mentions to reminder_text {nid_str}."
                )
            ...
            if isinstance(initial_delay, (int, float)) and initial_delay >= 0:
                ...
                try:
                    ...
                    state_to_save = {
                        ...
                        "reminder_text": final_context.get("reminder_text", ""),
